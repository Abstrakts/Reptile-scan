  ...
  <StackLayout class="card" width="300">
    <StackLayout class="card-top">
      <FlexboxLayout justifyContent="space-between">
        <Label class="card-title" text="Ajouter un repas" />
        <SVGImage
          @tap="$modal.close"
    ...
    <StackLayout class="card-content">
      <StackLayout class="added" v-if="added">
        ...
        <StackLayout>
          <Label text="Repas ajouté avec succès" />
        </StackLayout>
      </StackLayout>
      ...
      <StackLayout v-if="!added">
        <StackLayout>
          <GridLayout columns="250, auto" rows="50, 50" height="55">
            <TextField class="input" hint="Ajouter une date" row="0" col="0" v-model="animal.date" />
            <SVGImage
              class="setTime"
          ...
          <TextField class="input" hint="Quantité" v-model="animal.quantity" />
          <TextField class="input" hint="Type de proie" v-model="animal.prey" />
          <TextField class="input" hint="Poids de la proie" v-model="animal.weight" />
          <TextField class="input" hint="Commentaire" v-model="animal.comment" />
        </StackLayout>
      </StackLayout>
      <StackLayout class="confirm-button" @tap="addMeal" v-if="!added">
...
export default {
  ...
  methods: {
    addMeal() {
      if (this.animal.date && this.animal.prey !== null) {
        this.$axios.post(
          `/api/user/me/animal/${this.qrcode}/addMeal`,
          {
            date: this.animal.date,
            quantity: this.animal.quantity,
            prey: this.animal.prey,
