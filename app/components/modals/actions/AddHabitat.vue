  ...
  <StackLayout class="card" width="300">
    <StackLayout class="card-top">
      <FlexboxLayout justifyContent="space-between">
        <Label class="card-title" text="Ajouter un entretiens" />
        <SVGImage
          @tap="$modal.close"
    ...
    <StackLayout class="card-content">
      <StackLayout class="added" v-if="added">
        ...
        <StackLayout>
          <Label text="Entretiens ajouté avec succès" />
        </StackLayout>
      </StackLayout>
      ...
      <StackLayout class="confirm-button" v-if="!added" @tap="addHabitat">
  ...
  methods: {
    addHabitat() {
      if (this.animal.date && this.animal.comment !== null) {
        this.$axios.post(
          `/api/user/me/animal/${this.qrcode}/addHabitat`,
          {
