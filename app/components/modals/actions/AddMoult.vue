  ...
  <StackLayout class="card" width="300">
    <StackLayout class="card-top">
      <FlexboxLayout justifyContent="space-between">
        <Label class="card-title" text="Ajouter une mue" />
        <SVGImage
          @tap="$modal.close"
    ...
    <StackLayout class="card-content">
      <StackLayout class="added" v-if="added">
        ...
        <StackLayout>
          <Label text="Mue ajoutée avec succès" />
        </StackLayout>
      </StackLayout>
      ...
      <StackLayout v-if="!added">
        <StackLayout>
          ...
          <TextField class="input" hint="Longueur" v-model="animal.length" />
...
export default {
  ...
  methods: {
    addMoult() {
      if (this.animal.date && this.animal.comment !== null) {
        this.$axios.post(
          `/api/user/me/animal/${this.qrcode}/addMoults`,
          {
            date: this.animal.date,
            length: this.animal.length,
