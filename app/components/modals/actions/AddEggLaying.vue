  ...
  <StackLayout class="card" width="300">
    <StackLayout class="card-top">
      <FlexboxLayout justifyContent="space-between">
        <Label class="card-title" text="Ajouter une ponte" />
        <SVGImage
          @tap="$modal.close"
    ...
    <StackLayout class="card-content">
      <StackLayout class="added" v-if="added">
        ...
        <StackLayout>
          <Label text="Ponte ajouté avec succès" />
        </StackLayout>
      </StackLayout>
...
export default {
  ...
  methods: {
    addMoult() {
      if (this.animal.date && this.animal.quantity !== null) {
        this.$axios.post(
          `/api/user/me/animal/${this.qrcode}/addLayings`,
          {
    ...
    formatDate(format) {
      ...
      if (day.quantity === 1) day = `0${day}`;
      if (month.quantity === 1) month = `0${month}`;
